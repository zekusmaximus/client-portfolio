...
- [ ] Jeff, after deploy: the Data Upload page shows the new column list and "Download template"; a one-row test file naming a lead who is not on the People list (for example `CLIENT,Contract Period,Lead` / `Test Client,1/1/26-12/31/26,Nobody`) is refused with row 2 listed, and the client count in the header does not change. Do not import the template itself: its two example rows would be added to the book.
...
- [ ] Jeff, after deploy: the sheet in the section 3 format, with every associate it names and every former colleague it names as an originator on the People list (the former colleagues inactive).
...
- [ ] Jeff: `reset-book` without an argument, counts equal to the backup log's `clients` and `client_revenues`, every key line `ok`; then `--confirm`; record its result line here.
- [ ] Jeff: the sheet imported on gbacpod.com; the header's total equals the latest year's column sum; a few clients' leads, second chairs and revenue checked in their Edit Client forms; the People dialog's counts match the sheet.
