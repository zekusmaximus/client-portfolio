...
- [ ] `ci`, GitGuardian and `backup-selftest` green on the PR head; the Netlify Deploy Preview builds and its header rules check reports the headers (see the PR).
...
|---|---|---|---|
| 12.1 Removing an account can never remove clients | in progress (2026-09-25) | `claude/magical-mayer-je13sz`, no PR yet | `clients.user_id` is `ON DELETE SET NULL`; verified in the container on PostgreSQL 16, CI runs PostgreSQL 18. Jeff's: the deploy steps in 12.1 |
...
- [ ] The `ci` workflow's `check` and `schema` jobs, and `backup-selftest`, green on the PR.
- [ ] Backup run green before the merge. Run id:
