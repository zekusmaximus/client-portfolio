...
**Status:** approved plan; progress is in the section 2 status table. Written 2026-09-23 against commit `3c745a6`.
...
- No schema migrations are needed anywhere in Tier 0. If you find yourself writing `ALTER TABLE`, stop and re-read the WP. (Section 12 records Tier 2 schema work done early, outside the work packages.)
...
| D12 | Backups, in two layers. Layer 1: whatever Render provides for the database's instance type (backups, point-in-time recovery). Layer 2: a nightly `pg_dump --format=custom --no-owner --no-privileges` of the Render External Database URL (a read-only role preferred, `PGSSLMODE=require`), run by GitHub Actions in the private repository `zekusmaximus/client-portfolio-backups` and never in this one, with `postgresql-client-$PG_MAJOR` from apt.postgresql.org. Before anything is encrypted, the dump is restored into a throwaway `postgres:$PG_MAJOR` service container and the row count of every public table must equal production's; `users`, `clients` and `client_revenues` must exist and `users` must not be empty. Then `age` to one recipient, plaintext deleted, artifact kept 90 days. Schedule `17 7 * * *` plus `workflow_dispatch` (first `0 7 * * *`; moved off the hour on 2026-09-25 after its first night produced no run). Replaces the VPS design (systemd timer, Hetzner Storage Box, 30-day local retention), 2026-09-24. | Production is a Render web service and Render PostgreSQL, so there is no server to run a timer on. This repository is public, so no job that touches production data or credentials may run in it. Client data leaves Render only encrypted, and a restore needs one private key kept in the password manager. A dump that has not restored with matching counts is never kept, so a bad backup fails loudly instead of being discovered during a restore. |
| D13 | The page is built and served by Netlify (project `client-portfolio2`, from `main`); the API is the Render web service `client-portfolio-backend`, and Express stays API-only. A root `netlify.toml` pins the page build (repository root as base, `npm run build:prod`, publish `dist`, `NODE_VERSION` 22) and sets the page's headers: `Strict-Transport-Security: max-age=31536000; includeSubDomains` (no preload), `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-Frame-Options: DENY`, and `Content-Security-Policy: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self' https://client-portfolio-backend.onrender.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'`. A directive is relaxed only where the build requires it, and `script-src` never gets `'unsafe-inline'` or `'unsafe-eval'`. `VITE_API_BASE_URL` stays a Netlify UI build variable. No SPA fallback while there are no client-side routes. No `render.yaml` or Blueprint in Tier 0; `deploy/README.md` records the Render settings. Deploying is merging to `main`; rolling back is Netlify's publish-a-previous-deploy and Render's rollback. An external monitor checks `/api/health` for `"status":"OK"` every 5 minutes and alerts by email. Replaces the nginx design (nginx serving `dist/` and proxying `/api/`, a systemd unit, `deploy.sh`, `rollback.sh`), 2026-09-25. | Production has no server of its own: the page is on Netlify and the API on Render (confirmed 2026-09-24 in WP3), so the page's headers can only come from Netlify's configuration, and `connect-src` must name the API because the page calls another site. Linking a Blueprint can change the live services, so Render stays configured in its dashboard and documented in the runbook. |
...
|---|---|---|---|---|
| WP0 | Quality gates and the crash fix | deployed | `claude/tier0-wp0-quality-gates`, [PR #10](https://github.com/zekusmaximus/client-portfolio/pull/10) | baseline lint was 104 problems in 32 files (97 errors, `.cjs` included), not the 100 in 3.1; all errors fixed, the 7 `exhaustive-deps` warnings remain by design (D3). Deployed as part of WP2's deploy and every one since. On 2026-09-25 GitHub's branches API reported `main` unprotected, so CI reports on a PR but does not block its merge (section 10 item 5) |
| WP1 | Year-agnostic revenue | deployed | `claude/tier0-wp1-year-agnostic-revenue`, [PR #11](https://github.com/zekusmaximus/client-portfolio/pull/11) | no Postgres in the session container, so the database half of 4.4 is Jeff's on the server and `tests/strategic.test.mjs` stands in for the score regression; `calculateStrategicValue(client)` now honours `client.revenues` (argument resolution, the D6 formula is unchanged); `benchmark-csv.cjs` and one export footnote were hard-coded years that 4.1 missed. Deployed as part of WP2's deploy and every one since; the live half of 4.4 (import, re-import a single-year sheet, check `client_revenues`) is not recorded |
...
| WP3 | Server hardening | deployed | `claude/tier0-wp3-server-hardening`, [PR #13](https://github.com/zekusmaximus/client-portfolio/pull/13) | Hosting confirmed 2026-09-24: page on Netlify (gbacpod.com), API on Render (client-portfolio-backend.onrender.com) behind Render's proxy, Render PostgreSQL, env vars in the Render dashboard. **D10 deviation:** page and API are different sites, so a `Lax` cookie would never reach the API; the cookie is `SameSite=None; Secure` in production and `Lax` in development, 7 days for both JWT and cookie as D10 says, and the first 6.4 item reads `SameSite=None; Secure` in production. Where the plan and the code differed, the code won: (1) 6.1 line numbers were taken at `3c745a6`; `server.cjs` had moved down one line (WP2's import), the rest matched; (2) `callback(null, false)` and dropping the form parser do not stop a cross-site form POST from reaching a route, and the AI Advisor's analyze-portfolio and strategic-advice routes need no body (reproduced: a form POST from a foreign origin with a partner's cookie reached the Anthropic call), so `server.cjs` also answers POST/PUT/PATCH/DELETE from a foreign `Origin` with 403, in its own commit; (3) express-rate-limit 8.7.0 logs `ERR_ERL_KEY_GEN_IPV6` for a `keyGenerator` that falls back to `req.ip` without `ipKeyGenerator`, so the fallbacks use it, keys carry `user:`/`ip:` prefixes, and `loginIpLimiter` keeps the library's default key; with `X-Forwarded-For` present and `trust proxy` unset it logs `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` and keeps running, it does not refuse as 6.2.6 says; (4) `pg` applies an `sslmode` in `DATABASE_URL` over the `ssl` option, so `.env.example` drops `?sslmode=require`; an unrecognised `DATABASE_SSL` stops startup; (5) the error handler keeps body-parser 4xx statuses (413 over 5 MB, 400 for malformed JSON) instead of answering 500; (6) the password policy returns errors for non-string input instead of throwing, and `create-admin.cjs` also hashes through `utils/hash.cjs`; (7) `.env.example` also gains `NODE_ENV` and `FRONTEND_URL`; the variables 6.2.9 removes lived only in the two deleted files; (8) express-rate-limit brings `ip-address` and moves `debug` 4.4.1 → 4.4.3; (9) section 9's manual deploy assumes a VPS: on Render the variables WP3 needs are already set and deploying is a redeploy. D11 consequence: a succession run over more than 30 clients in an hour lists the rest as failed with the 429 text. Verified end to end in the container on Postgres 16 (PR); the production `Secure` flag and `TRUST_PROXY_HOPS=1` behind Render's real proxy are Jeff's after deploy. **Deployed:** on 2026-09-24 at 19:54 UTC, `/api/health` returned `status: OK`, `uptimeSeconds: 8509`, `database: connected`, `anthropic: configured`, `model: claude-opus-5` (run by Jeff; the WP4 session container cannot reach onrender.com). The 6.4 acceptance list has not been run on the live site yet |
| WP4 | Backups | deployed | `claude/tier0-wp4-backups`, [PR #14](https://github.com/zekusmaximus/client-portfolio/pull/14), follow-ups [PR #15](https://github.com/zekusmaximus/client-portfolio/pull/15) and [PR #16](https://github.com/zekusmaximus/client-portfolio/pull/16) (schedule moved to 07:17 UTC) | **Re-scoped 2026-09-24 (D12 rewritten, section 7 rewritten):** the plan's design assumed a VPS (systemd timer, Hetzner Storage Box, local retention). Production is a Render web service with Render PostgreSQL, so no host exists to run a timer, and this repository is public, so its Actions logs and artifacts cannot hold production data or credentials. Layer 1 is Render's own backups; Layer 2 runs in the private repository `zekusmaximus/client-portfolio-backups` from copied files. Where the plan and the code differed, the code won: (1) the counts and the dump share one exported snapshot (psql holds a REPEATABLE READ transaction, `pg_dump --snapshot`), so a write during the run cannot cause a false mismatch; the self-test proves it and fails when `--snapshot` is removed; (2) on PostgreSQL 16 a non-superuser table owner cannot `GRANT pg_read_all_data` ("Only roles with the ADMIN option"), so `INSTALL.md` falls back to table-level `SELECT` grants, verified to back up and to refuse writes; (3) beyond the brief, the script refuses a non-empty restore target (which also keeps a misconfigured `RESTORE_CHECK_URL` from writing into production), requires `pg_dump`, production and the restore-check server to share a major version, and refuses an `age` private key in `AGE_RECIPIENT` without printing it; (4) render.com and docs.github.com were unreachable from the session (proxy 403), so every Render-specific step reads "confirm in the Render dashboard" with the documentation URL, and no Render plan or retention numbers are stated; (5) `deploy/backup/.gitattributes` forces LF, so a copy made on Windows still runs under bash. Verified in the container on PostgreSQL 16; CI runs the self-test on 16, 17 and 18. Jeff's: instance type, key pair, private repository, secret and variables, first run, restore drill. **Installed 2026-09-24 (Jeff):** the Render database is Basic-256mb on PostgreSQL 18, created about a year earlier; the dashboard shows no point-in-time recovery, so Layer 2 is the only backup. The read-only role `client_portfolio_backup` is in use (its `SELECT` works and an `UPDATE` is refused). First manual run 36047210847 green: `client_revenues` 251, `clients` 100, `users` 7 (seven accounts against six partners), dump 16,986 bytes. First restore drill passed the same day (`RESTORE.md` (f)). Found during the install and fixed in the follow-up PR: commands that read the key from the clipboard cannot work when the command itself was just copied, so both documents now prompt for the key with `Read-Host -MaskInput`; `gh auth refresh -s workflow` is needed before the private repository's first push; a new terminal tab can keep the pre-install `PATH`; the first decrypted dump disappeared from disk before `pg_restore` (cause not established, antivirus suspected). Moves to `verified on gbacpod.com` once the first scheduled run is green and the Render environment values are in the password manager. **Checked 2026-09-25 (session):** the private repository's `backup.yml` has the 07:17 schedule (`f58cae7`, 11:21 UTC) but still uses `actions/checkout@v4` and `actions/upload-artifact@v4`, so WP5's v7 change has not been copied (`pg-backup.sh` is identical); runs 36128523698 (11:16 UTC) and 36137742528 (12:55 UTC) were manual and green; no scheduled run has happened yet, and the first one the 07:17 schedule can produce is 2026-09-26 |
| WP5 | Deployment as code, docs, cleanup | merged | `claude/tier0-wp5-deploy-docs-cleanup`, [PR #17](https://github.com/zekusmaximus/client-portfolio/pull/17) | **Re-scoped 2026-09-25 (D13 rewritten; sections 8, 9 and 10 rewritten):** the plan's WP5 assumed a VPS reached over SSH, with nginx, a systemd unit, `deploy.sh` and `rollback.sh`. Production is Netlify for the page and Render for the API and PostgreSQL, so there is no server to configure: the page's build and headers move into `netlify.toml`, the API stays API-only on Render with its settings recorded in `deploy/README.md`, and deploying is merging to `main`. No `render.yaml`, because linking a Blueprint can change the live services. Where the plan and the code differed, the code won: (1) the dead `express.static` for `../frontend/build` and the commented catch-all were already removed in WP3 (`845a608`), so `server.cjs` is unchanged; (2) the facts block came back blank: Netlify's settings were read through the Netlify connector (the live deploy of `0507a49` processed no header or redirect rules, so none are configured in the UI), and every Render web-service setting reads "confirm in the dashboard" in the runbook's section 1.2; (3) netlify.com, render.com, onrender.com and gbacpod.com were unreachable from the container (proxy 403, WebFetch blocked), so Netlify, Render and UptimeRobot facts come from search-engine excerpts of their documentation pages, cited by URL, and the runbook says so; (4) `node-fetch`'s only importer was `benchmark-csv.cjs`, deleted here, so it went with the other eight unused packages; (5) `npm audit fix` took 28 advisories to 5 (production dependencies 19 at the review, 3 now; minor and patch versions only); all five remaining need a major (`bcrypt` 6 for `tar`, Vite 8) and are in the runbook's section 12; (6) CI's warning named `actions/checkout@v4` and `actions/setup-node@v4`; checkout, setup-node and upload-artifact moved to v7, the newest major on 2026-09-25 (`node24`), and `deploy/backup/backup.yml` changed, so its copy in the private repository must be replaced; (7) the built page needed no CSP relaxation: in headless Chromium against a local replica of both hosts serving exactly the `netlify.toml` headers, sign-in, a CSV import, all six tabs, both CSV exports, the print window and an AI call raised no CSP violation; (8) `CLAUDE.md` said client data persisted in localStorage, but only UI state does; (9) `clients.user_id` is `ON DELETE CASCADE`, so deleting the seventh account would delete every client it imported: the runbook forbids deleting users and revokes access with a password reset; (10) Netlify's Deploy Preview Drawer loads a script from app.netlify.com that the CSP blocks, so a preview console can show one CSP error that gbacpod.com never does; (11) `npm run build:prod`'s POSIX `NODE_ENV=` prefix does not run under npm's `cmd.exe` on Windows, so the README gives `npx vite build`. Jeff's: the 8.3 items marked (Jeff). **Checked 2026-09-25 (session):** `ci`, GitGuardian, `backup-selftest` and Netlify's header rules check green on the PR head; Netlify published the production deploy of `4d89f41` (which contains WP5) to gbacpod.com at 13:00 UTC with 1 header rule processed. Render's deploy and the 8.3 (Jeff) items are not recorded |
...
- **Layer 2: a nightly logical backup in GitHub Actions**, in the private repository only, at `17 7 * * *` (07:17 UTC, about 3:17 am Eastern) and on `workflow_dispatch`. It was `0 7 * * *` until 2026-09-25, when its first night produced no scheduled run; GitHub delays scheduled runs at the top of the hour and can drop them under load. `deploy/backup/backup.yml` runs `deploy/backup/pg-backup.sh`. Both files are copied into the private repository, so the job that holds production credentials never runs code fetched from this public one. The job:
...
- [x] Instance type, PostgreSQL major version and Layer 1 features confirmed in the Render dashboard (`INSTALL.md` step 1) and written into section 2; a Free instance upgraded.
- [x] Key pair generated; the private key is in the password manager and `INSTALL.md` step 4's round trip printed `round trip ok`.
- [x] `zekusmaximus/client-portfolio-backups` exists, is private, and holds `pg-backup.sh`, `.github/workflows/backup.yml` and `.gitattributes`.
- [x] Secret `BACKUP_DATABASE_URL` (a read-only role where Render allows one) and variables `PG_MAJOR` and `AGE_RECIPIENT` set; artifact retention at least 90 days; failure emails on.
- [x] First manual run green, its log shows `row counts match`, and the artifact downloaded.
- [x] Restore drill per `RESTORE.md` (a) to (d): counts match the run's log; first row of the drill table filled in.
...
**Re-scoped 2026-09-25.** The first version of this section assumed a VPS: facts gathered over SSH, an nginx site config, a systemd unit, `deploy.sh` and `rollback.sh`, and an uptime monitor on `https://gbacpod.com/api/health`. Production has no such server. The page is built and served by Netlify, and the API is a Render web service with Render PostgreSQL (`CLAUDE.md`, "Hosting and deploying"), so the page's headers can only come from Netlify's configuration and there is nothing to install on a host. D13 records the replacement decision; this section describes what was built. WP4 re-scoped D12 the same way.
**Effort:** one session; about an hour of Jeff's time for the checks in 8.3 and the uptime monitor.
**Expected outcomes:**

- The repository and the runbook are everything needed to rebuild the Netlify site and the Render services: `netlify.toml` (the page's build and headers), `.env.example` (every server variable), and `deploy/README.md` (each service's settings, the order to rebuild them, and the data restore through `deploy/backup/`).
- Deploying is merging to `main`: Netlify builds and publishes the page, and Render deploys the API when its auto-deploy is on. Rolling back is one action per provider: publish a previous Netlify deploy, or roll back a Render deploy.
- The page is served with real security headers (HSTS, CSP, nosniff, frame denial, referrer policy) from `netlify.toml`, and API calls that take a minute or more are not cut off: the page calls Render directly, not through Netlify, and Render's documentation states that web-service HTTP responses may take up to 100 minutes (<https://render.com/docs/render-vs-vercel-comparison>, <https://render.com/docs/heroku>; read through search excerpts, the container could not open render.com). That is far beyond the Anthropic client's own limit (`services/anthropic.cjs`: 180-second timeout, 2 retries).
- An external uptime monitor checks `https://client-portfolio-backend.onrender.com/api/health` for `"status":"OK"` every 5 minutes and alerts by email.
- `CLAUDE.md` describes the system as it is; the stale status documents are archived; `README.md` explains the real deployment.

### 8.1 Facts from the dashboards

A session needs Netlify and Render settings, not server output, and Jeff supplies them by name only, never secret values. For Netlify: base directory, build command, publish directory, Node version, build variable names, any headers or redirects set in the UI, the domains and certificate status, where the DNS is hosted. For Render: instance type, region, Node version, root directory, build, start and pre-deploy commands, auto-deploy and its branch, health check path, variable names; the database's instance type, version and region; any existing uptime monitoring. Anything not supplied is written as "confirm in the dashboard", as `deploy/backup/INSTALL.md` step 1 does.

On 2026-09-25 the facts block came back empty. Netlify's settings were read through the Netlify connector instead (the live deploy of `0507a49` processed no header or redirect rules, so none are configured in the UI). The Render web service's settings are in the runbook's section 1.2 as "confirm in the dashboard", for Jeff to fill in.

### 8.2 Deliverables

1. **`netlify.toml`** (D13): `command = "npm run build:prod"`, `publish = "dist"`, no `base` (Netlify's default, the repository root), `NODE_VERSION = "22"`, and the five headers for `/*`. No SPA fallback: nothing calls `pushState` or reads `location`, react-router is gone, and the tab lives in the store, so a refresh always requests `/`. The built `dist/` was checked for what the CSP could block (inline scripts, external fonts or scripts, `eval`, `blob:` workers, `data:` URLs) and needed no relaxation.
2. **`deploy/README.md`**, the runbook, with PowerShell for everything Jeff runs. It covers hosting and each service's settings; setting each up from scratch; deploying, verifying and rolling back, with a one-time rehearsal; backups and restore by link to `deploy/backup/INSTALL.md` and `RESTORE.md`; adding a partner and resetting a password from the Render Shell or locally; rotating `JWT_SECRET` and the Anthropic key; reading the logs (`ai_call`, `ai_error`, `rate_limited`, `origin_refused`, `pg_pool_error`); `/api/health`'s fields; the uptime monitor, including what a Free instance's spin-down means; and the npm audit advisories that remain.
3. **`.env.example`**, the single environment template: `NODE_ENV`, `PORT`, `DATABASE_URL`, `DATABASE_SSL`, `JWT_SECRET` (left empty, with a generation hint that works in PowerShell), `SESSION_TTL`, `FRONTEND_URL`, `TRUST_PROXY_HOPS`, `BCRYPT_SALT_ROUNDS`, `ANTHROPIC_API_KEY`, `AI_MODEL`; `VITE_API_BASE_URL` documented as a Netlify build variable.
4. **API:** Express stays API-only and keeps its own security headers. The dead `express.static` for `../frontend/build` and the commented catch-all this item was to remove were already gone (WP3, `845a608`).
5. **Cleanup:** `git mv` of `CODE_REVIEW.md`, `CSV-UPSERT-IMPLEMENTATION.md`, `DASHBOARD-STATUS-UPDATE.md`, `GROWTH-SCENARIO-FIX.md`, `HTTPS-DEPLOYMENT-GUIDE.md`, `JAVASCRIPT-ERROR-FIXES.md`, `JWT-SECURITY-MIGRATION.md`, `PERFORMANCE.md`, `PRACTICE-AREA-FIX.md`, `REFACTOR-SUMMARY.md`, `REMOVAL_LOG.md`, `REVENUE-CONTRACT-FIXES.md`, `SECURITY-VALIDATION.md`, `SETUP-SECURITY.md`, `VALIDATION-SECURITY-IMPLEMENTATION.md`, `Client Portfolio Optimization Dashboard.md`, the `DELIVERY COMPLETE` file, `Pasted_content_01.txt` and `todo.md` into `docs/archive/` (D14). Deleted `benchmark-csv.cjs`, `scripts/verify-ai-cache-perf.cjs`, `scripts/security-check.js` with its `security:check` script entry, and `src/debug-console.js` with its import in `src/main.jsx`. Removed `multer`, `jwt-decode`, `uuid`, `dompurify`, `react-router-dom`, `class-variance-authority`, `react-dropzone`, `p-limit` and `node-fetch` after grepping each (`isomorphic-dompurify` stays for `src/utils/validation.js`). Ran `npm audit fix`, non-breaking only. `PRODUCT_BRIEF.md`, `REVIEW-2026-09.md`, `README.md`, `CLAUDE.md` and `docs/plans/` stay where they are.
6. **Workflows:** `actions/checkout`, `actions/setup-node` and `actions/upload-artifact` moved from v4 (Node 20, deprecated on GitHub's runners) to v7 in `ci.yml`, `backup-selftest.yml` and `deploy/backup/backup.yml`.
7. **`README.md`** rewritten: what it is, running it locally, how it is deployed, where the plan lives. **`CLAUDE.md`** brought in line: the six tabs, every endpoint, the commands, hosting and deploying.

### 8.3 Acceptance

The session's, verified in the PR:

- [x] Section 0.3 gates after the cleanup: lint 0 errors (7 warnings, D3), 62 tests pass, the production build exits 0.
- [x] `npm ls --depth=0` shows none of the removed packages.
- [x] `ls *.md` at the repository root shows only `CLAUDE.md`, `PRODUCT_BRIEF.md`, `README.md` and `REVIEW-2026-09.md`.
- [x] The built `dist/index.html` and assets contain nothing the CSP blocks. Headless Chromium was run against a local replica of both hosts that served exactly the `netlify.toml` headers and the API in production mode. Sign-in, a CSV import, all six tabs, both CSV exports, the print window, an AI call and a reload raised no CSP violation.
- [x] `ci`, GitGuardian and `backup-selftest` green on the PR head; the Netlify Deploy Preview builds and its header rules check reports the headers (see the PR).

Jeff's:

- [ ] Before merging, on the Deploy Preview: the header rules check lists the five headers, the preview builds and loads, and the sign-in page's console shows no CSP error. The preview cannot sign in, because its `*.netlify.app` origin is not `FRONTEND_URL` and the API refuses it (runbook 4.1). An error from Netlify's Drawer script is expected there and does not occur in production (runbook 4.4).
- [ ] Right after merging, on gbacpod.com: runbook 4.4's PowerShell prints all five headers; sign in, open every tab (Data Upload, Dashboard, Client Details, Partnership, AI Advisor, Scenarios) with the console open, and see no CSP error. If anything is blocked, publish the previous Netlify deploy (runbook 5.1).
- [ ] `/api/health` after Render has deployed the merge: `status: OK`.
- [ ] Uptime monitor configured (runbook 11.1) and one test alert received (11.2).
- [ ] Rollback rehearsal done once for Netlify and Render (runbook 5.4).
- [ ] `deploy/backup/backup.yml` copied into `zekusmaximus/client-portfolio-backups` (INSTALL.md step 5; it now uses checkout and upload-artifact v7), and the next run is green.
- [ ] The Render settings in runbook section 1.2 looked up and written in.

---

## 9. Deploying a merged WP

Deploying a merged work package is merging it to `main`. Netlify builds and publishes the page; Render deploys the API if its auto-deploy is on (otherwise use Manual Deploy on the service page). Then:

```powershell
Invoke-RestMethod https://client-portfolio-backend.onrender.com/api/health | ConvertTo-Json
```

`status` must be `OK`. Run that WP's acceptance list on the live site and set its status in section 2 to `deployed`, then to `verified on gbacpod.com` when the list passes. `deploy/README.md` section 4 has the full check (the deploy logs, the headers, the browser console) and section 5 the rollback. Environment changes a WP needs are made on the Render service's Environment page before or with the merge; WP3's (`TRUST_PROXY_HOPS=1`, `SESSION_TTL=7d`) are already set.
...
3. Login is rate-limited per username, the cookie is `SameSite=None; Secure` in production (`Lax` in development) with the foreign-origin refusal that makes it safe (WP3, D10 as adjusted for Render), partners can change their own passwords, and a reset is one command.
4. A nightly encrypted backup exists off-box and one restore drill has succeeded.
5. The repository and the runbook (`deploy/README.md`) can rebuild the Netlify site and the Render services from scratch, merging to `main` deploys, CI blocks broken PRs, and `npm run lint && npm test` pass on `main`.
...
Tier 0 allows no schema change (0.2). The items here are Tier 2 work (section 11) done ahead of the Tier 2 plan on Jeff's instruction, one branch each; that plan, when written, lists them as done.
...
**Problem.** `init-db.sql` created `clients.user_id INTEGER REFERENCES users(id) ON DELETE CASCADE`, and `client_revenues.client_id` cascades from `clients`, so deleting a user deleted every client whose `user_id` named it, and those clients' revenue rows. The book is one shared book for six equal partners (`PRODUCT_BRIEF.md`), so who created a client is incidental. The WP4 install found seven accounts for six partners; tidying up the extra one could have deleted real data. WP5's runbook forbade deleting users (a procedure); this makes the database keep the clients.

**What `user_id` is used for (checked 2026-09-25).** Nothing scopes by it: `e27e086` (2025-07-23, "all users see all data") removed every `user_id` clause from `data.cjs` and `models/clientModel.cjs`, and `claude.cjs` still passes `userId` to `clientModel.listWithMetrics` and `getWithMetrics`, which ignore it. No current code writes it: the CSV import (`data.cjs` pass 2b), `POST /api/data/clients` and `PUT /api/data/clients/:id` leave it out; the last writer, the unused `routes/clients.cjs`, went in `527e848` (2026-06-18). It reaches the page inside `c.*`, and nothing in `src/` reads it. `deploy/backup/selftest.sh` sets it in synthetic rows. So only clients created before July 2025 carry it; `npm run check:schema` prints how many per account.

**Decision: `ON DELETE SET NULL`, not dropping the column.**

1. A Render rollback (runbook 5.2) runs the older `init-db.sql` at start. That file indexes `clients(user_id)`, so on a database without the column it fails with `column "user_id" does not exist` (reproduced) and the production process exits: dropping the column would turn every rollback past this change into a failed start. After `SET NULL`, the older file applies cleanly and leaves the key as `SET NULL` (reproduced).
2. The column is the only record of who created the pre-July-2025 clients. Whether Tier 2's `updated_by` and change log replace it is that plan's decision, and a dropped column comes back only from a restore.

**Changes.**

- `init-db.sql`: `CREATE TABLE clients` names the key `clients_user_id_fkey` with `ON DELETE SET NULL`, so a new database never has the cascade. A `DO` block at the end finds every foreign key from `clients.user_id` to `users` whose action is not `SET NULL`, by column rather than by name (so a cascading key under another name is caught too), drops it, and adds `clients_user_id_fkey ... ON DELETE SET NULL` when none remains. After the first start it is a catalog read that changes nothing (the constraint's oid is stable across starts). `server.cjs` sends the file as one multi-statement query, which PostgreSQL runs as one transaction, so a failure anywhere in the file leaves the old key in place (reproduced by appending a failing statement). On the first start the `ALTER TABLE` briefly locks `clients` and `users` and checks 100 rows.
- `utils/schemaCheck.cjs` (pure): the SQL that lists the foreign keys to `users`, and `checkUserForeignKeys(rows)`.
- `scripts/check-schema.cjs` (`npm run check:schema`): read-only; prints the keys and the clients per creating account, exits 1 when any key cascades.
- `scripts/delete-user.cjs` (`npm run delete:user -- <username>`): one `REPEATABLE READ` transaction that refuses while any key cascades, refuses an unknown username and the last account, and commits only if the client and revenue-row counts are unchanged.
- `tests/schema.test.mjs` and a `schema` job in `.github/workflows/ci.yml` (PostgreSQL 18, as production). The database tests need `SCHEMA_TEST_SERVER_URL` and are skipped without it, so the `check` job skips them.
- `deploy/README.md` 7.3 rewritten: no warning; check, then delete with the script; the password reset stays as the way to revoke access and keep the account. Section 7's introduction mentions it. `CLAUDE.md` updated (commands, suites, the `init-db.sql` line, file map, CI).
- `deploy/backup/` is unchanged, so the copies in the private backup repository need nothing.

**Evidence (session container, PostgreSQL 16.13, 2026-09-25).**

- Reproduced on the pre-change `init-db.sql`: with a user who created 3 of 5 clients, `DELETE FROM users` left 2 clients and 4 of 10 revenue rows.
- `tests/schema.test.mjs`: 7 of 7 pass. With the pre-change `init-db.sql` in place, all 4 database tests fail.
- `deploy/backup/selftest.sh` with the new `init-db.sql`: 14 passed, 0 failed (the `backup-selftest` workflow runs it on 16, 17 and 18 because `init-db.sql` changed).
- End to end (0.4): a database built with the pre-change file, two accounts, four clients created by the second with 2025 revenue. `check-schema` printed `CASCADES` and exited 1; `node server.cjs` printed `Database tables initialized`; `check-schema` then printed `ON DELETE SET NULL` and `OK` and exited 0. Signed in as the first account, `GET /api/data/clients` returned 4 clients totalling $200,000 for 2025; `delete-user extra` printed `Clients: 4, of which 4 had been created by this account ... Revenue rows: 4. Accounts left: 1.`; the same request, with the same session, returned the same 4 clients and total, each with `user_id` null.
- `npm run lint` (0 errors, the 7 expected warnings), `npm test`, and the production build pass.

**Deploying it (Jeff).**

1. Back up first: run the backup by hand and wait for it to go green (`deploy/backup/INSTALL.md` step 7, from `gh workflow run backup.yml --repo zekusmaximus/client-portfolio-backups` to `gh run watch`). Record the run id below. `deploy/backup/RESTORE.md` restores from it if needed.
2. Before merging, from a checkout of this branch with runbook 7.2's environment, run `node scripts/check-schema.cjs` against production. Expected: `CASCADES  clients.clients_user_id_fkey: FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE`, the clients per creating account (what deleting that account would have taken), `FAIL`, exit 1. A different key name is handled by the migration; no key at all means production differs from `init-db.sql` and is worth a look before merging.
3. Merge. Render deploys the API. The start-up log must show `Database tables initialized` (runbook 9.2). `Database initialization failed` means the transaction left the database as it was and the new process exited, so the deploy should fail and leave the old instance serving (runbook 4.2; check the service's Events). Bring the log line to the next session.
4. Run `node scripts/check-schema.cjs` against production again: it must end `OK` and exit 0. Then `/api/health` (section 9).
5. Only after step 4, and only if wanted: remove the seventh account with runbook 7.3.

**Acceptance.**

- [x] The `ci` workflow's `check` and `schema` jobs, and `backup-selftest`, green on the PR.
- [x] Backup run green before the merge. Run id: 36137742528 (12:55 UTC; the merge was at 13:00 UTC).
- [ ] `check-schema` on production: `FAIL` before the deploy, `OK` after.
- [ ] `/api/health` returns `status: OK` after the deploy.
