...
| WP3 | Ask the book and the brief | 1.5 sessions | merged | `claude/focused-goldberg-u95rkh`, [PR #34](https://github.com/zekusmaximus/client-portfolio/pull/34) | Gates green: lint 0 errors (the 3 expected warnings); `npm test` 281 pass (254 before); database suites 75 pass on PostgreSQL 16 (71 before: WP1's test of the three AI Advisor routes out, three WP3 tests in, each on both shapes); `claude.cjs` (635 lines) deleted. End to end in headless Chromium on production's table shape with a 12-client book, 7 rated: 26 of 26 checks (section 8's acceptance). `ASK_INSTRUCTIONS` is 5,443 characters, about 1,360 tokens at 4 per token, so with production's 84-client book (17,637 characters) the cached prefix is about 5,800 tokens, an estimate, above `claude-opus-5`'s 512-token cache minimum (the `claude-api` skill). Where the plan and the code differed, the code won: (1) the answer's `model` is the model requested (`AI_MODEL`, as in the `ai_call` line and WP4's `model` column), `servedBy` the one that answered; (2) `models/clientModel.cjs`'s `get` went with `getWithMetrics`, its only caller, beyond section 8's letter; (3) the question is trimmed at its ends and otherwise sent as written; the 400s are "Type a question to ask." and "The question is too long: at most 2,000 characters."; (4) today's date is the firm's day, America/New_York, as WP4 takes the month; (5) the tab is the header, the WP2 panel, the Ask card, then the answers; (6) the declined notice adds the refusal's category, as T10 says; (7) the AI limiters run before the question check, so a refused question counts (T16); (8) `import-db`'s WP3 tests are one before the first import (the empty book) and two at the end, never between Phase 3's and WP2's; (9) the fake server's default answer names the system blocks, the marker, the book's client rows and the question, as section 4 describes. Found, not changed: the `claude-api` skill lists `claude-opus-5-5` as launching ($4 / $20, default effort `medium`, no price in `utils/aiCost.cjs`, not in `FALLBACK_MODELS`): a D7 decision for Jeff; the runbook's sections 9.3 and 10 predate WP1's log fields and three feature names. Left: Jeff's after-deploy items in section 8 |
| WP4 | Saved answers and cost | 1 session | PR open | `claude/focused-bardeen-t6uiyb`, [PR #35](https://github.com/zekusmaximus/client-portfolio/pull/35) | Gates green: lint 0 errors (the 3 expected warnings); `npm test` 296 pass (281 before: 14 in the new `ai-answers` suite, 1 in `schema`); database suites 87 pass on PostgreSQL 16 (75 before: 8 in `schema`, 2 in `import-db` on each shape). End to end in headless Chromium on production's table shape with a 12-client book and 24 earlier answers: 24 of 24 checks (section 9's acceptance). Every new test was run against `main`'s server and `init-db.sql` in a worktree (with the test files and the pure `utils/aiAnswers.cjs` they import) and failed for the right reason. Where the plan and the code differed, the code won: (1) `answerRow` also takes `model`, the model requested (`AI_MODEL`): `complete()`'s result carries only the model that served; (2) the month is two bounds computed in JavaScript (`firmMonth`, `Intl` in America/New_York, unit-tested across the daylight-saving changes) and compared with `created_at` in SQL (`created_at >= $1 AND created_at < $2`), not `AT TIME ZONE` in the SQL; (3) `asked_by` is written as `(SELECT id FROM users WHERE id = $6)`, so a session that outlived its account (the JWT is not checked against `users`) saves with the username and a null `asked_by` instead of failing the foreign key; (4) the list pages by `(created_at, id)` of the `before` row, asks for one row more to answer `hasMore`, takes `limit` 1 to 50 (400 otherwise) and names the 200 characters `preview`; the summary is `{ month, timeZone, from, to, answers, costUsd, unpriced }`; `cost_usd` and `costUsd` stay text as pg returns `NUMERIC`, and the page parses them (`src/utils/recentAnswers.js`); (5) two files beyond section 9's list: `models/aiAnswerModel.cjs` (the database side, shared by `routes/ai.cjs` and `routes/scenarios.cjs`; `saveAnswer` never throws) and `src/utils/recentAnswers.js` (the page's pure helpers, tested); (6) the transition plan's response adds `answerId` and `saved` beside `plan` (T14 keeps the rest), so a failed save is reported there as for Ask; the page reads only `plan`; (7) a transition plan stores `reporting_year` from the request's `stage1Data.reportingYear` and no `book_sha256` until WP6; (8) `answerRow` never throws: a kind outside the table's `CHECK` fails the save like any failed save; (9) the response's `costUsd` keeps six decimals and the row four (`NUMERIC(10, 4)`; PostgreSQL rounds); the month's total sums the stored figures; (10) the AI tab also shows Recent answers on an empty book, so answers saved before a reset stay readable, and an answer card says when its answer was not saved; (11) the backup grant is in `deploy/backup/INSTALL.md` step 2 ("Optional: a read-only role for the backup"), not step 3 as section 9 said; corrected there. Left: Jeff's before-merge backup item and after-deploy items in section 9 |
...
Measured on production on 2026-09-26, with WP1's service on the pre-WP3 `client-recommendations` prompt for one client (section 6): 2,483 input tokens, and 11,698 and 12,978 output tokens with the thinking; $0.30 and $0.34, 96% of it output; 163 and 179 seconds. On that prompt the output ran two to three times the 1,000 to 5,000 assumed above. WP3's answers ask for about 300 words (the brief about 600), so its first `costUsd` figures (section 8) and WP4's saved costs are what to judge Ask, the brief and `AI_EFFORT` (T11) by.
...
- [x] Jeff, after deploy: `/api/health` reports `anthropic: configured` and `model: claude-opus-5`; one AI Advisor question answers; the Render log's `ai_call` line shows `servedBy`, `cacheReadTokens` and `costUsd`, and the Console's usage for that minute is in the same range as `costUsd`. 2026-09-26, Jeff: health as required; a client recommendation for one client answered at length; the Console showed $0.64. Closed 2026-09-26 with the two `ai_call` lines Jeff sent for that client (label `client-recommendations`, the route WP3 later deleted): both show `servedBy` `claude-opus-5`, `fellBack` false, `cacheReadTokens` 0 (that route set no cache marker) and `costUsd`; 2,483 input and 11,698 output tokens, $0.304865 in 163 s, and 2,483 and 12,978, $0.336865 in 179 s. Each `costUsd` recomputes from its tokens at $5 and $25 per million, and the two sum to $0.64173, the Console's $0.64.
...
- [ ] Jeff (PR #31's item 5, from the stickiness fix): 83 of the 84 clients read Stickiness 3 on 2026-09-26, the one Jeff rated by hand aside, so the old backfill had rated every client the sheet left blank. Once PR #33 is deployed (the client form keeps "Not rated" instead of filling 3), clear them to "not rated" with the guarded Render Shell command in PR #33's description: it counts first, clears every client at 3 in one transaction, and changes nothing unless exactly the expected number would change. The panel's Exposure section should then count them under "Not rated".
...
- [x] Gates and both database suites green; `wc -l` shows `claude.cjs` (635 lines) gone. Done 2026-09-26: lint 0 errors, `npm test` 281 pass, both database suites 75 pass on PostgreSQL 16, the production build green, and `claude.cjs` deleted with its 635 lines.
- [x] End to end in the container as above; no console error from this change. Done 2026-09-26 on production's table shape with a 12-client book, 7 of them rated: the header read "Rated for stickiness: 7 of 12"; an example question filled the box; Ask and Brief rendered the fake's answers, which named the book's 12 client rows and two system blocks with the marker on the second; a tab switch kept both answers; a question with `&`, an apostrophe and `<` reached the fake as typed; every request sent the same system blocks; no client note reached it; a refusal showed the notice with its category; `/api/health` routed to WP2's feature list showed "The API has not been updated yet; try again in a few minutes." with no Ask, Brief or example buttons and sent nothing; one `ai_call` line per request. No console error after sign-in (the two `GET /api/auth/me` 401s before it predate this change).
...
**Backups (Jeff, before the merge).** The backup role reads new tables only if it was given `pg_read_all_data` or the default privileges in `deploy/backup/INSTALL.md` step 2. `people` (Phase 1) was the first table created after the role; if every nightly backup since Phase 1 merged is green, `ai_answers` needs nothing. If one failed with `permission denied for table people`, grant as INSTALL.md says before this package deploys, or the first night after it fails too. A backup job that uses the database's own URL (step 2 skipped) reads every table and needs nothing.
...
- [x] Gates and both database suites green. Done 2026-09-26: `npm ci` clean, lint 0 errors (the 3 expected warnings), `npm test` 296 pass, both database suites 87 pass on PostgreSQL 16, the production build green.
- [x] End to end in the container as above. Done 2026-09-26 on production's table shape, in headless Chromium against `server.cjs`, the fake server and Vite, with two partners in separate browser contexts, a 12-client book and 24 earlier answers seeded (2 of them last month): the first partner's Ask and brief headed Recent answers and were saved with that partner; the second partner saw both at the top and opened the Ask in full (the question, the fake's answer, who asked, the tokens and the cost); "Older answers" loaded the rest, 26 rows for 26 answers, newest first as the database orders them, and then went away; a seeded transition plan read "Transition plan: Smith & O'Brien Holdings"; "This month: 24 answers, about $0.73" matched the saved costs this month ($0.7269, which the summary returned exactly); after a refresh the list came back with the answer just given, which opened from it; with `/api/health` routed to a list without `ai-answers`, the tab offered Ask with no Recent answers, no error and no request to `/api/ai/answers`; one `ai_call` line per request and no `ai_answer_save_failed` line; no console error after sign-in.
