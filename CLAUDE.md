...
- `routes/ai.cjs` - `GET /api/ai/book` (WP2), `POST /api/ai/ask` and `POST /api/ai/brief` (WP3); the AI limiters only on the POSTs
...
- `parseResponse()` collects text by content-block type, skipping `thinking` and
  ...
  `{ success: true, plan: { ..., truncated, refused }, timestamp }`;
