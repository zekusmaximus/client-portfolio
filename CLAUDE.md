...
- Succession plans are in-memory only (review section 7; server-side persistence is Tier 2). The Partnership tab's departing flags, redistribution models and custom reassignments are gone (Phase 4); the store's `partners`/`fetchPartners`, derived from the legacy text, remain only for the Scenarios workflow until Phase 5
