...
- Current suites: `people` (the People list's validators, the P5 change rules, the legacy-field shim, which also drops the retired `status`, the page's picker helpers and the client list's person filters), `load` (Phase 4: each person's lead book and second-chair load, the second chair's 20% effort share (P10 as amended), the role averages over active people, the ratios and "—" where there is no peer, the lead-book order, an inactive person only while in a seat, and the page's effort equal to the server's `getEffort` for every cadence; its fixture book is `tests/fixtures/books.mjs`, shared with `book`), `book` (Tier 1 WP2, `utils/book.cjs`: parity with the page's `partnershipModel` at the page's reporting year on the fixture book and 200 seeded random books, every person's counts, revenue, effort and ratios within 1e-9 and shown identically; `reportingYear` equal to `computeReportingYear`; the two `SECOND_CHAIR_EFFORT_SHARE` values equal; the same text for shuffled clients and people and for two `now` values in one reporting year; every client once, sorted by name regardless of case then id as text, integer and uuid ids; `&amp;` decoded; no note, id, date, `user_id` or retired column; "not rated" and "not set" for defaults, an unrated client under unrated exposure; the legend's formula equal to the scorer's; an inactive person with a seat listed and marked; the empty book), `partnership-exports` (the load CSV row by row, CSV quoting and formula guarding, the report built from the model with `strategicValue`, escaped names and the second-chair share in its effort columns), `smoke` (scoring formula), `strategic` (score regression fixture, D6), `csv-import` (year rule, D5, and each year's total; a `Contract Period` column changes nothing and raises no issue, P13), `import-sheet` (the import sheet's people and judgment columns, P8: header matching, each vocabulary, name resolution, every refusal, blank cells clearing, a file without the new columns reading and writing as before, `public/client-book-template.csv` against the plan's section 3 example, stored clients keyed by their name unescaped, and a name two stored clients share refusing its row), `escaping` (`utils/escaping.cjs`: `ESCAPES` is `validator.escape`'s set; `unescapeText` undoing one escape; `unescapeStored` undoing 0 to 4 escapes of seeded random text without `;`; `unescapeStoredSql` built from the same replacements in the same order; the page's `src/utils/escaping.js` equal to it on any text, `;` included), `client-form` (`src/utils/clientForm.js`: a stored client's name and notes filled unescaped and every other field as before; an escaped name passing the form's name pattern; and, modelled as the page's `sanitizeFormData` then the server's `validator.escape`, saving again storing the same text, including notes with a `<`, which DOMPurify escapes first; a client nobody has rated filled as Not rated (`stickiness` null) and a rating kept), `book-sheet` (`src/utils/bookSheet.js`: the header against the template's columns, each cell and vocabulary, escaped names and notes written unescaped, quoting and no BOM in the text, several revenue years with gaps and a year with only a `$0` row left out, a missing or inactive lead or an inactive second chair written as stored and listed, and the sheet read back by `processCSVData` and `checkSheet` into every stored value, with only the listed rows refused), `reporting-year` (D4), `ai-service` (Tier 1 WP1: the real SDK driven through `createService({ apiKey: 'test', fetch })` by the recorded streams in `tests/fixtures/anthropic/`: thinking then text, `max_tokens`, a refusal before output and one mid-stream, a fallback served mid-stream and before output, the whole chain refusing, cache writes, an unpriced model, `onText` in order and a throwing `onText`; the recorded request (streamed, the system blocks unchanged, the fallback beta and `"default"` on `claude-opus-5` only, `output_config.effort` only when set, nothing else); `AI_EFFORT` validation and `server.cjs` refusing to start on a bad value; `ANTHROPIC_BASE_URL` honoured outside production and ignored in it; 429, 401, 500, 529, 400, a mid-stream `overloaded_error` and a network failure through `describeError`; no request without a key; and the timeout bounding only the wait for an answer to start, on Node's own fetch), `ai-cost` (WP1, `utils/aiCost.cjs`: the price table, cache writes by lifetime and reads, a fallback priced per attempt at each model, the refusal billing rule, no guess for an unpriced model, the normalised counts), `transition-plan` (per-client prompt and parser, D9; the request check, which takes production's integer client ids; Phase 5b: the roster check against the People list, the prompt's roster and its two seat sections, the parser resolving each recommendation against the roster only, a name off the roster resolving to nobody, and no invented timeline), `transition-plans` (Phase 5b, `src/utils/transitionPlans.js`: the request's roster, the accepted plan as an import sheet of CLIENT, Lead and Second Chair, who still holds a seat after it, approval blockers, Stage 3's transitions with no invented dates or rates, and a client name the form stored escaped written unescaped and passing the CLIENT pattern), `departure` (Phase 5's departure engine: who needs a new lead or second chair, each seat's candidates in order by total effort, the promoted second chair's seat, a departing emeritus or associate, several people at once, no candidate, the partner's choices applied or refused with the reason, P3 after every departure on random books, the before and after loads with the 20% share, the defaults spreading a book, accepting a default moving no other, and the store's two scenario helpers), `succession-utils` (the transition complexity reads `interaction_frequency` and the conflict label), `associate-split` (Phase 6, `src/utils/associateSplit.js`: the clients with a lead and no second chair, heaviest first, an associate proposed for each by fit then total load, proposals spreading, the picker's associates then everyone else P3 allows, a pick applied or refused with the reason, a client whose lead is not an active partner shown but not proposed for, each associate's load before and after against the associates' average, P3 on random books, and no associates), `ai-grep` (the grep assertions, widened in Tier 1 WP1: no retired model ids, sampling parameters or thinking budget; one module that calls `messages.create(`, `messages.stream(`, `countTokens(` or `new Anthropic(`; `betas`, `fallbacks` and `output_config` only in the service, and `output_config` only as `output_config: { effort }`; and, from WP3, no `claude.cjs` and no `/claude/` in `src/` or `server.cjs`), `ask-prompts` (Tier 1 WP3, `utils/askPrompts.cjs`: the system blocks byte-identical for two questions and the brief, the instructions then the book with the one cache marker on the book, the question and the date only in the user turn and the question unescaped, today's date in America/New_York, the brief's five headings in order, `checkQuestion`'s limits, each T6 rule and T5's definition in the instructions by a key phrase, and no date, name or "check your work" line in them; then the page's `src/utils/askTheBook.js`: the brief's three example questions, the box's length equal to the server's, and "Rated for stickiness" counting what the book counts as rated), `ai-answers` (Tier 1 WP4, `utils/aiAnswers.cjs`: `answerRow` for a plain, a truncated, a refused (before output and mid-stream) and a fallback-served result from the real service and the recorded streams, every column the insert names, the question stored as written, a transition plan's client id as text and its name with any number of escapes undone, the book's SHA-256, the cost and the date its prices were read as `complete()` gave them and a null cost for an unpriced model, and no throw; the kinds equal to the table's `CHECK`; the SQL's untyped placeholders, its column order and newest-first order; `readListQuery` and `readAnswerId`; `firmMonth` across the daylight-saving changes; then the page's `src/utils/recentAnswers.js`: costs parsed from text, a row's title, the month's line, older pages appended once), `password-policy` and `session-ttl` (WP3), `schema` (the pure `checkClientForeignKeys` and `checkBookReset`, and `reset-book`'s usage error without a database; `init-db.sql` on a real PostgreSQL: `clients.user_id` is `ON DELETE SET NULL` on a new database and after migrating one with the old cascade, deleting a user keeps its clients and revenue rows, and the two scripts above, Tier 0 plan section 12; the `people` table's seed, uniqueness and role check, the client foreign keys and the second-chair check, a pre-plan database migrating intact, and a rollback start running the pre-plan file on a migrated database; an unrated client staying unrated, and a rating staying as picked, across starts, and a database from before the `stickiness` column seeded once from `relationship_intensity` and then left alone (Tier 1 WP2); `reset-book` printing the counts and deleting nothing, emptying a seeded book with the users and people rows unchanged, counting and emptying V2's `revenues` table through its cascade, refusing and naming a key that does not cascade, and succeeding on an empty book; Tier 1 WP4's `ai_answers`: its columns, `kind` check, key to `users` (`ON DELETE SET NULL`) and index on a new database, a second start changing nothing and leaving saved answers as they are, a pre-WP4 database migrating with its book intact, a rollback start running the pre-WP4 file leaving the table and its rows, `check-schema` still ending `OK`, `delete-user` keeping an account's answers with `asked_by` null and the username kept, and `reset-book --confirm` leaving them untouched), `import-db` (run twice: on the tables `init-db.sql` creates, and on production's older tables with integer ids, created before `init-db.sql` runs, each run first checking its id type, then Tier 1 WP3's Ask and brief answering 400 "The book has no clients yet." on the empty book; `POST /api/data/process-csv` on `server.cjs` and a real PostgreSQL: the section 3 example with the People counts, the clients' people and the stored legacy text; a refused file leaving clients, revenue and people byte-for-byte unchanged; a file without the people columns leaving them untouched; blank cells clearing; a name with an apostrophe resolving through the request sanitizer; a kept second chair who would equal a new lead; Check file (`dryRun`) writing nothing and answering exactly what the import answers; then Phase 3's flow: `reset-book` on the suite's database, the book empty and every People count 0 in the same session, and the section 3 template imported into the empty book; then P13: a file with only `CLIENT` and the years and the same file with a `Contract Period` column writing identical rows, inserted and updated, with Check file writing nothing, the stored `status` never written by the import or by a `PUT` or `POST` carrying a stray `status`, and no client response carrying one; then Phase 5: `POST /api/scenarios/transition-plan` with this run's client id type passing every check to the 503 of a missing key, and refusing a roster naming someone not on the People list or someone leaving; and a transition sheet of only `CLIENT`, `Lead` and `Second Chair` changing the two seats and nothing else, after which the partner who left can be deactivated; then Phase 6: `PUT /api/data/clients/:id/second-chair` setting one seat and its legacy text with every other column, revenue row and person unchanged, answering 409 when the seat changed since the page loaded, 400 for the lead, an inactive or an unknown person or a malformed body, 404 for a missing client of either id type, and clearing the seat; and Tier 1 WP2, right after Phase 3's template import: `GET /api/ai/book` listing every client once, each listed person's lead and second-chair counts equal to the People list's, no note and no uuid in the text, and 401 without sign-in; last, a client whose name the form stored escaped: `unescapeStoredSql` equal to `unescapeStored` on PostgreSQL, a sheet with `&` and an apostrophe imported, both clients re-saved through `PUT /api/data/clients/:id` (one twice, sending its stored name), then Check file and the same sheet updating both at their ids with `summary.newClients` 0 and the sheet's spelling written back, and a name two stored clients share refused with nothing written until one is deleted; then the page's side: that client saved through `PUT` as the form sends it (`clientFormData`, `sanitizeFormData`) three times with its name and notes stored unchanged after the first and, saved for another reason, still not rated, and a transition sheet from the page's `departureModel` and `buildTransitionSheet` for it passing Check file and importing, one client updated and none created; last, the book as a sheet from the page's `buildBookSheet` over the API's clients and People list: the clients without a lead refused by Check file with nothing written, then, once each has a lead, Check file and Upload updating every client and creating none, every client byte-for-byte unchanged but `updated_at` and the text the sanitizer stored escaped, every revenue row unchanged by client, year and amount, every person unchanged, and a second download identical and importing with every byte the same; last, Tier 1 WP3 on the whole book: without a key, `POST /api/ai/ask` and `/api/ai/brief` reading the book on this table shape and stopping at the 503, an empty, blank, missing or non-string question and a 2,001-character one answering 400 (2,000 passing), 401 without sign-in, the AI limiters' `RateLimit-Policy` header on the POST and none on `GET /api/ai/book`, and the three `/api/claude` routes answering 404; then a second `server.cjs` on the same database with `ANTHROPIC_API_KEY=test` and `ANTHROPIC_BASE_URL` at `startFakeAnthropic()`: Ask and the brief returning the fake's text, the recorded request carrying `max_tokens` 32,000, the instructions and the book `GET /api/ai/book` shows as its two system blocks with the marker on the book, every client once in the book, a question with `&`, an apostrophe and `<` only in the user turn as typed, the brief and a second question sending the same blocks byte for byte, and a refusal answering `refused` with no text and its category; last of all, Tier 1 WP4's saved answers: on the keyless server, WP3's four answers saved with the signed-in account and nothing saved from the requests that stopped at a check or the missing key, the list's fields newest first, 401 without sign-in and no AI limiter on the three GETs, 404 for an id that is not a positive integer or not found and 400 for a bad `limit` or `before`; then on a second `server.cjs` pointed at the fake: an Ask saved with every column (who asked, the question as written, the tokens, the cost as `NUMERIC(10, 4)` stores the response's estimate, the book's hash), the brief, a transition plan for a client the form stored escaped saved with its id as text (digits on production's shape, a uuid on the other) and its name unescaped, still readable after the client is deleted, a fallback-served, a cut-off and a refused answer, an AI error not saved, a second partner's answer saved as theirs and the same list for both, the list paging with `before` to every answer once, this month's summary in America/New_York leaving out an answer an instant before the month began, and a failed save (a `CHECK (false) NOT VALID` added for the test) returning the answer and the plan with `saved: false` and one `ai_answer_save_failed` line each). Functions that depend on the clock take a `now` argument (`computeReportingYear(clients, now)`) so tests are deterministic
...
- **API Routes**:
  ...
  - `POST /api/ai/ask` `{ question }` and `POST /api/ai/brief` `{}` - Ask the book and the brief, the AI tab (`routes/ai.cjs`, Tier 1 WP3): behind `authenticateToken` and the two AI rate limiters, no request sanitizer (the question is trimmed and otherwise sent as written). `checkQuestion` answers 400 for anything but 1 to 2,000 characters; an empty book answers 400 `The book has no clients yet.`; then one `complete()` call with the shared system blocks (`systemBlocks`, `utils/askPrompts.cjs`) and `maxTokens` 32,000, answering `{ success, id, saved, kind, question, answer, truncated, refused, refusalCategory, servedBy, model, usage, costUsd, reportingYear, timestamp }` (`model` is the one requested, as in the `ai_call` line; `question` null for the brief); errors through `describeError`, and never saved. The answer is saved first (WP4): `id` is the saved row's, and a failed save answers as usual with `saved: false`, `id` null and an `ai_answer_save_failed` log line
  - `GET /api/ai/answers?before=<id>&limit=20`, `GET /api/ai/answers/summary` and `GET /api/ai/answers/:id` - the saved answers (`routes/ai.cjs`, Tier 1 WP4; `models/aiAnswerModel.cjs`, SQL in `utils/aiAnswers.cjs`), every partner's, behind `authenticateToken` with no AI rate limiter. The list is newest first by `created_at` then `id`: `{ success, answers: [{ id, kind, question, client_name, asked_by_username, created_at, served_by, truncated, refused, cost_usd, preview }], hasMore }`, `preview` the answer's first 200 characters; `limit` 1 to 50 (default 20), `before` the last id shown, anything else 400. The summary is this calendar month in America/New_York: `{ success, timeZone, month, from, to, answers, costUsd, unpriced }`, the month's bounds computed by `firmMonth` and compared with `created_at` in SQL; `unpriced` counts answers with no cost, which the sum leaves out. `:id` answers `{ success, answer }` (the row, `prices_read_on` as `YYYY-MM-DD`), 404 for an id that is not a positive integer or not found; `summary` is registered before it. `cost_usd` and `costUsd` are text (pg returns `NUMERIC` as a string); the page parses them (`parseCost`, `src/utils/recentAnswers.js`)
  - `POST /api/scenarios/transition-plan` `{ client, stage1Data: { departing, impactData, reportingYear }, roster }` - one succession transition plan for one client (`routes/scenarios.cjs`); the roster of people staying is checked against the People list before the model is called (400 with the names otherwise). The plan is saved as a `transition-plan` answer (WP4: `client_id` `String(client.id)`, `client_name` with the request sanitizer's escaping undone by `unescapeStored`), and the response adds `answerId` and `saved` beside `plan`
...
1. **Data Upload** (`DataUploadManager.jsx`, `data-upload`) - CSV import through a file picker, parsed in the browser by PapaParse and posted to `/api/data/process-csv`; lists a refused file's problems by row, shows the sheet's columns (plan section 3) and links `public/client-book-template.csv`. **Download the book as a sheet** writes the whole book as an import sheet from the store's clients and People list (`buildBookSheet` in `src/utils/bookSheet.js`; "The book as a sheet", under Import sheet below). **Check file** posts the same file with `dryRun: true` and writes nothing; it answers `The file is ready to import: ...` without the updated/new split, which before a reset is measured against the old book
2. **Dashboard** (`DashboardView.jsx`, `dashboard`) - Analytics and visualizations
3. **Client Details** (`ClientListView.jsx` + `ClientEnhancementForm.jsx`, `client-details`) - Client management. The form's Relationship Stickiness has a sixth choice, "Not rated" (`stickiness` null): a client nobody has rated opens with it and keeps it when saved for another reason, and a new client starts with it (`clientFormData` in `src/utils/clientForm.js`). Until 2026-09-26 the form filled an unrated client with 3, so any save rated it 3, which the book counts as safe (T5)
...
`CLIENT` named twice regardless of case or shared by two stored clients (below),
which refuses every file, with or without the new columns. Contract periods no longer produce `validation.issues`;
the list stays in the response and still does not block.
**Client names** match stored clients regardless of case and unescaped. A client
saved through the client form is stored HTML-escaped by `sanitizeRequestBody`
(`Barnes &amp; Noble`, `O&#x27;Brien`; escaped again by each save that sends the
stored text back), so the import selects the rows with
`LOWER(unescapeStoredSql('name')) = ANY($1)` and keys them with `unescapeStored`
(`utils/escaping.cjs`; one list of replacements, and `import-db` checks the SQL
and the JavaScript agree on PostgreSQL). Nothing else compares a stored name.
A matched client is updated and its name written in the sheet's spelling, so an
import repairs the stored names of the clients it names; no migration rewrites
names (`init-db.sql` runs at every start, and each pass would strip another
layer), and the escaped text the sanitizer left in notes and elsewhere is Tier
2's, with the sanitizer. A `CLIENT` two stored clients share this way (for
example the duplicate the old exact match created) refuses its row: the import
cannot tell which one to update, and a partner deletes one on Client Details.
The page undoes the same escaping with its copy of `unescapeStored`
(`src/utils/escaping.js`, held equal by the `escaping` suite; the page never
imports the server's modules): the client form fills its name and notes with it
(`clientFormData` in `src/utils/clientForm.js`), so an edit saves without
retyping the name and a save no longer adds a level of escaping, and Scenarios'
transition sheet writes client names with it, so Check file and the import read
them. The list and other views still decode one level for display
(`formatClientName`, `src/utils/textUtils.js`).
**The book as a sheet.** Data Upload's "Download the book as a sheet" builds, in
the page and without an API call, the sheet the import reads with every client's
current values (`buildBookSheet` in `src/utils/bookSheet.js`, pure;
`exportBookSheet` downloads `client_book_<date>.csv`), so that importing it
unchanged changes nothing; it is the master sheet for a new year's revenue or a
bulk correction. Columns: `CLIENT`, a `YYYY Contracts` column for every year in
which any client has an amount above 0 (`revenueYearsOnFile`), then the ten
optional columns in section 3's order. Each cell is what the import reads back
into the stored value: amounts bare (`40000`, `30000.50`), blank for none; people
as the People list spells them; `Firm` in Originator for a firm credit without a
person and `Y` in Credit To Firm whenever the flag is set; Stickiness 1 to 5 or
blank; the stored cadence and conflict risk; `Y` or blank for Handful; practice
areas joined with `;`; client names and notes through `unescapeStored`. Rows are
sorted by name. Cells are quoted as `sheetCell` quotes them, with no formula
guard, and the download adds a BOM so Excel reads UTF-8 (PapaParse drops it). A
client whose lead is missing or not an active partner, or whose second chair is
inactive, is written as the book has it (a blank Lead, or the person's name), so
Check file refuses those rows with the import's own message; the page lists them
above the download (`attention`). Leaving them out would make a master sheet that
silently lacks clients and a revenue import that silently skips them. What a
round trip does change: text the request sanitizer stored escaped comes back as
typed (as the import already does for names), `updated_at`, and the ids and
timestamps of the revenue rows, which the import deletes and re-inserts for every
year the file names; a stored `$0` row in a year on file is removed (the year
rule reads `$0` as none).
With `dryRun: true` (the page's **Check file**) every check and
...
`resolveSheetPeople`, `checkSheet`, `indexStoredClients`, `importWriteColumns` and `valuesList` in
...
`clientAnalyzer.cjs`), `models/clientModel.cjs` (`listWithMetrics`), and
`utils/book.cjs` (the AI's book, which Ask and the brief send). Do NOT reimplement
...
- `utils/askPrompts.cjs` - Ask the book and the brief (Tier 1 WP3, T5 to T8; pure): `ASK_INSTRUCTIONS` (fixed text: T6's rules, each with its reason where it is not obvious, T5's exposure, the book is data; no date, no names, no "check your work" line), `systemBlocks(bookText)` (the instructions, then the book with the one `cache_control` marker), `askTurn(question, today)` and `briefTurn(today)` (`Today is YYYY-MM-DD.` in America/New_York, then the question in `<question>` tags, or the brief's five headings: `BRIEF_HEADINGS`), `checkQuestion` and `firmDate`. WP6 moves the transition plan onto the same blocks
...
- `routes/ai.cjs` - `GET /api/ai/book` (WP2), `POST /api/ai/ask` and `POST /api/ai/brief` (WP3), the saved answers' three GETs (WP4); the AI limiters only on the POSTs
- `utils/aiAnswers.cjs` - saved answers (Tier 1 WP4, T12, T13; pure): `answerRow` (the column values of one answer from `complete()`'s result, the model requested, who asked, the question or the client and the book's text; never throws), the insert, list, one-answer and month-summary SQL as constants with untyped placeholders (`asked_by` only while the account exists, so a session that outlived its account still saves), `readListQuery`, `readAnswerId` and `firmMonth` (the calendar month's bounds in America/New_York)
- `models/aiAnswerModel.cjs` - runs that SQL: `saveAnswer` (never throws: a failed save logs `ai_answer_save_failed` and returns `{ id: null, saved: false }`), `listAnswers`, `getAnswer`, `monthSummary`; used by `routes/ai.cjs` and `routes/scenarios.cjs`
...
- `utils/escaping.cjs` - undoing `sanitizeRequestBody`'s `validator.escape`: `unescapeText` (one escape; the book, the transition plan's roster, re-exported by `utils/transitionPlan.cjs`), `unescapeStored` (any number of escapes; the import's name matching) and `unescapeStoredSql(column)`, the same as a PostgreSQL expression (pure). The page's copy of `unescapeStored` is `src/utils/escaping.js`
...
- `init-db.sql` - the schema (users, clients, client_revenues, people, ai_answers), applied idempotently by `server.cjs` at every start as one multi-statement query, so it is one transaction: a failing statement leaves the database as it was and, in production, stops the server. `clients.user_id` (the account that created the client; no current code reads or writes it) is `ON DELETE SET NULL`: deleting a user keeps its clients and clears their `user_id`. The `DO` block at the end migrates a database that still has the old `ON DELETE CASCADE`, matching the key by column, and is a catalog read on every later start. The stickiness backfill (seeding `stickiness` from the legacy `relationship_intensity`) runs only in the start that adds the `stickiness` column: until Tier 1 WP2 it ran at every start, and since `relationship_intensity` is 5 on every new row (its default, and the import writes 5), each unrated client became a 3 at the next start. Never write a data backfill in this file that can match rows created after it first ran; a rollback to an older file runs that file's statements, the old backfill included. Keep migrations in this file idempotent, and do not drop a column an older `init-db.sql` still names: a Render rollback runs the older file at start (plan section 12). `ai_answers` (Tier 1 WP4) holds every AI answer: `client_id` is `TEXT` with no foreign key to `clients` (production's ids are integers, this file's uuids, and `reset-book` refuses while any key to `clients` does not cascade), `asked_by` references `users` `ON DELETE SET NULL` with the username copied beside it, `cost_usd` is `NUMERIC(10, 4)` with the date the prices were read, and `book_sha256` records the book an answer was given on
...
- `src/utils/bookSheet.js` - the book as an import sheet (`buildBookSheet`, `revenueYearsOnFile`, `exportBookSheet`; pure but for the download), for Data Upload's "Download the book as a sheet"
- `src/utils/escaping.js`, `src/utils/clientForm.js` - the page's `unescapeStored`, and `clientFormData`, the client form's fields for a stored client with its name and notes unescaped (pure)
...
- `src/utils/askTheBook.js` - the AI tab's example questions, the box's length and the header's rated-for-stickiness count (WP3; pure)
- `src/utils/recentAnswers.js` - the AI tab's Recent answers (WP4; pure): `parseCost` (the API's `NUMERIC` text), `formatCost`, `answerTitle`, `monthLine`, `appendAnswers`
...
- **Self-test:** `.github/workflows/backup-selftest.yml` runs `deploy/backup/selftest.sh` on PostgreSQL 16, 17 and 18 with synthetic data, the happy path and every failure path. Run it locally against a throwaway server only (it creates and drops `pgb_selftest_*` databases): `SELFTEST_SERVER_URL=postgresql://postgres@127.0.0.1:5432 bash deploy/backup/selftest.sh`. A new table in `public` is counted automatically (`ai_answers`, Tier 1 WP4, needed no change), provided the backup role can read it (`pg_read_all_data`, or the default privileges in `deploy/backup/INSTALL.md` step 2); renaming `users`, `clients` or `client_revenues` means updating the required list in `pg-backup.sh`.
...
- The AI tab's answers live in the store (`aiResults`, `{ ask, brief }`, each the route's whole answer, and `aiError`; not persisted, cleared on logout) so switching tabs, which unmounts the tab content, does not discard a paid answer. Whether a request is in flight is the component's state
- The AI tab's saved answers live in the store too (`aiAnswers`: the list's `items`, `hasMore`, `loaded`, `loadingOlder`, `error`, this month's `summary`, the answers opened as `details` by id, and `openId`; not persisted, cleared on logout), loaded by `fetchAiAnswers` (the first page and the summary), `fetchOlderAiAnswers` and `toggleAiAnswer`. A refresh loses nothing: the list comes back from the API
...
- `parseResponse()` collects text by content-block type, skipping `thinking` and
  ...
  and keeps the raw `usage`. `complete()` adds `costUsd` and `pricesReadOn`. Ask
  and the brief answer as `POST /api/ai/ask` above; a transition plan answers
  `{ success: true, plan: { ..., truncated, refused }, answerId, saved, timestamp }`;
...
- Saved answers (Tier 1 WP4, T12, T13): every route that calls `complete()`
  saves its answer in `ai_answers` after the call returns, through
  `answerRow` (`utils/aiAnswers.cjs`) and `saveAnswer`
  (`models/aiAnswerModel.cjs`): the kind, the question or the client, the
  answer (empty for a refusal, with its category), who asked, the model
  requested and the one that served, the flags, the tokens by kind, the
  cost and `prices_read_on` as `complete()` gave them, the book's SHA-256,
  the reporting year and the call's duration. An AI error is logged, not
  saved. A failed save never loses an answer the partner has paid for: the
  route returns it with `saved: false` and logs one
  `{"event":"ai_answer_save_failed",label,userId,kind,code,message}` line. A
  new AI route saves its answers the same way, with a new `kind` in the
  table's `CHECK` and `KINDS`. Nobody can edit or delete a saved answer in
  Tier 1; every partner sees every one.
- Testing without a key (`docs/plans/tier-1.md` section 4):
  ...
  with a short text naming what it received (the system blocks and which one
  carries the cache marker, the book's client rows and the question, when
  there are any; a transition-plan request gets a plan in the six sections,
  recommending people from the prompt's roster);
...
- Ask the book and the brief (Tier 1 WP3, T6 to T8): one question per
  request with no history (T7), or the brief under T6's five headings. Both
  send the same two system blocks, `ASK_INSTRUCTIONS` then the book with the
  one `cache_control: { type: 'ephemeral' }` marker (the default 5-minute
  lifetime), and put today's date and the question after it, in the user
  turn, so a second question or the brief within five minutes reads the book
  from the cache (`cacheReadTokens` in the `ai_call` line; the minimum
  cacheable prefix on `claude-opus-5` is 512 tokens). Never put a date, a
  name, a count or anything per request into the instructions or the system
  blocks. The instructions state each T6 rule in the partners' words with its
  reason, including "never tell the partners to move a client" and its one
  exception, a request that says someone is leaving and asks for new seats,
  which WP6's transition plan relies on; `tests/ask-prompts.test.mjs` fails
  when a rule's key phrase goes. No "double-check your answer" line: the
  current model checks its own work, and telling it to makes it over-verify
  (the `claude-api` skill). Length is a prompt instruction (about 300 words,
  the brief about 600), not an effort setting.
...
- Deleted in Tier 1 WP3, do not resurrect: `claude.cjs` and its mount, the
  AI Advisor's three consulting-deck routes (`/api/claude/analyze-portfolio`,
  `/strategic-advice`, `/client-recommendations`) and their prompts, the
  tab's Portfolio Analysis, Strategic Advice and Client Insights panels,
  `generatePortfolioSummary` (`utils/strategic.cjs`) and `get` and
  `getWithMetrics` (`models/clientModel.cjs`). `tests/ai-grep.test.mjs` fails
  if `claude.cjs` returns or `src/` or `server.cjs` names a `/claude/` route.
- The AI tab renders answers with `react-markdown` (default settings, no raw
  HTML plugin), saved ones included, and keeps them in the store
  (`aiResults`, `aiAnswers`).
- The book (Tier 1 WP2, `utils/book.cjs`, `GET /api/ai/book`) is the text
  Ask and the brief send, as the second, cached system block (T8). It
