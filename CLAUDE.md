...
4. **Partnership** (`PartnershipAnalytics.jsx`, `partnership`) - who's carrying what (plan Phase 4, P10): the partners' lead books and everyone's second-chair load by role, as clients, reporting-year revenue and effort, each against the average of the active people in the same role (`partnershipModel` in `src/utils/load.js`; no capacity ceiling, no health score). The lead carries a client's full effort and the second chair 20% of it (`SECOND_CHAIR_EFFORT_SHARE` in `src/utils/load.js`, P10 as amended by Jeff on 2026-09-25); counts and revenue count in full for both chairs, and each client's own `effort` and strategic score are unchanged. Every second-chair effort figure reads that one constant; if the server ever needs it, mirror it in a CommonJS module with a parity test, as `tests/load.test.mjs` pins `resolveEffort` to `getEffort`. A name opens that person's clients (`components/PersonLoadSheet.jsx`); Export gives a printable report and everyone's load as CSV (`src/utils/partnershipExports.js`, pure builders). No departure modelling here: that is Scenarios. The Dashboard's "Heaviest lead book" card (`heaviestLeadBook`) links here, and the client list (Client Details) shows each client's lead and second chair and filters by either (`personFilterOptions`, `matchesPersonFilter` in `src/utils/people.js`)
5. **AI Advisor** (`AIAdvisor.jsx`, `ai`) - Claude API integration for strategic advice
6. **Scenarios** (`ScenarioModeler.jsx`, `scenarios`) - the three-stage succession workflow (`src/components/succession/SuccessionScenario.tsx`); the Growth and capacity scenarios were deleted in WP2 (D8). Stage 1 (`ImpactAnalysisWorkbench.jsx`, plan Phase 5) lists every active person by role as someone who may leave and runs the departure engine, `departureModel` in `src/utils/departure.js` (pure): each client a leaving person leads needs a new lead, an active partner who is staying (the client's second chair first when a partner who is staying, then practice-area fit with the candidate's lead book, then the lighter lead load: effort, revenue, name); each seat a leaving person holds as second chair, and the seat of a second chair promoted to lead, is emptied and gets candidates (everyone active and staying but the lead, by practice-area fit with every client they hold, then the lighter second-chair load with the 20% share). "Lighter" counts what the scenario has already given each person: clients are settled heaviest effort first, in a fixed order, so accepting a default moves no other. The partner's choices (`{ [clientId]: { leadId, secondChairId } }`) override the defaults; a choice P3 or the departures forbid is refused with its reason (`assignmentProblems`) and the default stands; no candidate leaves the seat empty and says so. Stage 1 shows the affected clients with their proposals and everyone's lead and second-chair load before and after (the leaving at zero, and out of the role averages). `rankCandidates`, `secondChairPool`, `createLedger` and `areaIndex` are Phase 6's building blocks. Stage 2 (`ClientReviewInterface.jsx`) and Stage 3 (`TransitionPlanManager.jsx`) are rebuilt in PR 5b
...
- Scenario state lives in the store (plan P11), so a tab switch keeps it: `successionWorkflow` (`currentStage`: `impact`, `mitigation` or `implementation`; `departingIds`; `choices`, the partner's picks per client, set with `setDepartureChoice`), `transitionPlans` (Stage 2's plans) and Stage 3's execution state. Not persisted (`partialize`), cleared on logout and by `resetSuccessionWorkflow`; saving scenarios on the server is later work (review section 7, Tier 2). The Partnership tab's departing flags, redistribution models and custom reassignments are gone (Phase 4); the store's `partners`/`fetchPartners`, derived from the legacy text, remain only for Stage 2's successor pickers and Stage 3's assignees until PR 5b
