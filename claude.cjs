...
// Every Anthropic call goes through services/anthropic.cjs (one client, one
...
// Shape every AI answer the same way: the text under `key`, plus the flags the
// UI needs to say "cut off" or "declined", the model that answered, and usage.
function aiResponse(key, result, extra = {}) {
  return {
    success: true,
    [key]: result.text,
...
function sendAiError(res, context, error) {
  ...
  console.error(context, expected ? error.message : error);
...
router.post('/strategic-advice', async (req, res) => {
  try {
    ...
    const { system, prompt } = createStrategicPrompt(portfolioSummary, query, context);
    const result = await complete({ system, prompt, userId, label: 'strategic-advice' });

    res.json(aiResponse('advice', result));
  } catch (error) {
    sendAiError(res, 'Error generating strategic advice:', error);
  }
});
...
router.post('/analyze-portfolio', async (req, res) => {
  try {
    ...
    const portfolioSummary = generatePortfolioSummary(clients);
    const { system, prompt } = createAnalysisPrompt(portfolioSummary);
    const result = await complete({ system, prompt, userId, label: 'portfolio-analysis' });

    res.json(aiResponse('analysis', result));
  } catch (error) {
    sendAiError(res, 'Error analyzing portfolio:', error);
  }
});
...
router.post('/client-recommendations', async (req, res) => {
  try {
    ...
    const { system, prompt } = createClientRecommendationPrompt(client, portfolioContext);
    const result = await complete({ system, prompt, userId, label: 'client-recommendations' });

    // `client` (the name) stays in the response: the AI Advisor keeps answers in
    // the store across tab switches and needs the name after a remount.
    res.json(aiResponse('recommendations', result, { client: client.name }));
  } catch (error) {
    sendAiError(res, 'Error generating client recommendations:', error);
  }
});
...
function createStrategicPrompt(portfolioSummary, query, context) {
  ...
  const system = "You are a senior strategic advisor with 20+ years of experience in government relations law firm management. You're preparing advice for the firm's executive committee, known for their analytical rigor and strategic thinking.";

  const basePrompt = `<firm_profile>
Portfolio Metrics:
...
function createAnalysisPrompt(portfolioSummary) {
  const system = 'You are a senior strategic consultant specializing in government relations law firms, with deep expertise in practice management, client portfolio optimization, and succession planning. Your analysis will be reviewed by managing partners and executive committees.';
...
function createClientRecommendationPrompt(client, portfolioContext) {
  ...
  const system = "You are a senior client relationship strategist for a premier government relations law firm. You're developing a comprehensive client strategy that will be reviewed by the managing partner and the client relationship partner.";

  const prompt = `<client_intelligence>
Client: ${client.name}
